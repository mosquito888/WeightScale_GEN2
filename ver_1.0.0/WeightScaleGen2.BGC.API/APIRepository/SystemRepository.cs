...
namespace WeightScaleGen2.BGC.API.APIRepository
{
    public class SystemRepository : ISystemRepository
    {
        ...
        public async Task<List<SystemData>> Select_SystemData(UserInfoModel userInfo)
        {
            using var conn = await _db.CreateConnectionAsync();

            var query = $@"EXEC sp_select_system_data";

            var datas = conn.Query<SystemData>(query).ToList();

            conn.Close();

            _logger.WriteActivity(message: string.Format(Constants.LogsPattern.Inquiry, this.GetType().Name, "Select_SystemData", null), user: _context.HttpContext.Session.GetString(Constants.Session.User));
